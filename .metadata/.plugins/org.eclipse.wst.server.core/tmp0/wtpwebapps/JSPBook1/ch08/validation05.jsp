...
function checkMember() {
	let regExpId = /^[a-z|A-Z|ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/;	// 영문 대문자/소문자, 한글, 한글의 자음과 모음만 시작
	let regExpName = /^[가-힣]*$/; // 이름은 한글만 사용가능  ^: 시작, $ : 끝, *0이상 반복
	let regExpPasswd = /^[0-9]*$/; //비밀번호는 숫자만 사용 가능
	let regExpPhone = /^\d{3}-\d{3,4}-\d{4}$/; // 전화번호 형태만 가능 , \d{3}: 숫자 3개, \d{3,4} : 숫자가 3이상 4이하
	let regExpEmail 
	...
	// Regular Expression의 메소드 중 test() =>  boolean 으로 리턴
