...
<body>
	...
	<form action="form02_process.jsp" name="member" method="post">
		<p>아이디:
			...
			<input type="button" value="아아디 중복검사" />
		</p>
		...
		<!--  체크박스를 선택한 경우 on이, 선택하지않은 경우는 null 이 전송됨 -->
		<p>취미:독서 <input type="checkbox" name="hobby" value="독서" checked/>
			운동 <input type="checkbox" name="hobby" value="운동"/>
			영화 <input type="checkbox" name="hobby" value="영화" />
		</p>
		<p>
			<!--  wrap="off" : 줄 바꿈 안함. soft : 자동 줄바꿈(기본)
					hard : 서버 전송 시 캐리지 리턴문자를 전달함
			 -->
			<textarea name="comment" rows="3" cols="30" 
			placeholder="가입인사를 입력해주세요" wrap="soft">		
			</textarea>
		</p>
