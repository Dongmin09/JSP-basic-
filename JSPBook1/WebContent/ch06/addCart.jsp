...
<%
	...
	//2-2) cartlist라는 장바구나가 없으면 장바구니를 생성
	...
	//		P1235라는 상품 객체(productVO)의 quantity 멤버 변수의 값을 1 증가
	for(int i=0; i<list.size(); i++){
		// list는 장바구니 안의 상품목록(P1234, P1235, P1236)
		// list => List<ProductVO>
		// list.get(0) = ProductVO
		// list.get(0).getProductId => P1234
		// String id = request.getParmeter("id"); // P1235
		if(list.get(i).getProductId().equals(id)){
			cnt++;
			// 장바구니에 상품이 이미 들어 있으므로 장바구니에 담은 개수에 1 증가
			// list.get(i) => productVO(상품)
			// list.get(i).setQuantity() 상품의 수랑을 세팅
